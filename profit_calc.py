...
import pandas as pd
import numpy as np
from utils.store import load_all, INVOICE_TYPES
...
def _extract_amount(df: pd.DataFrame, label: str) -> pd.Series:
    """Find and clean the amount column."""
    from utils.parser import detect_amount_column
...
def _extract_sku(df: pd.DataFrame) -> pd.Series:
    from utils.parser import detect_sku_column
...
def build_pnl(date_start=None, date_end=None) -> pd.DataFrame:
    """
    Build a per-SKU P&L DataFrame combining all invoice types.
    Returns columns: sku, revenue, fba_fees, shipping, storage,
    ...
    for itype, df in data.items():
        ...
        amounts = _extract_amount(df, itype)
    ...
    # Pivot: rows = SKU, cols = invoice_type
...
def build_daily_trend(date_start=None, date_end=None) -> pd.DataFrame:
    """Daily revenue, cost, profit time series."""
    data = load_all()
    if not data:
        return pd.DataFrame()
    ...
    pivot["total_cost"] = (
        pivot.get("fba_fees", 0)
        + pivot.get("shipping", 0)
        + pivot.get("storage", 0)
        + pivot.get("advertising", 0)
        + pivot.get("returns", 0)
    )
...
def get_totals(date_start=None, date_end=None) -> dict:
    ...
    if trend.empty:
        return {k: 0 for k in
                ["revenue", "fba_fees", "shipping", "storage",
                 "advertising", "returns", "total_cost", "profit", "margin_pct"]}
