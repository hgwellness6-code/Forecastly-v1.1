...
from utils.profit_calc import build_pnl, build_daily_trend, get_totals
from utils.store import load_invoice, INVOICE_TYPES
...
if totals["advertising"] > rev * 0.3:
    st.warning("📣 Advertising is consuming more than 30% of revenue (ACoS too high).")
if totals["returns"] > rev * 0.15:
    st.warning("↩️ Returns are above 15% of revenue. Check product quality / listings.")
...
with col_bar:
    ...
    summary_bar = pd.DataFrame({
        ...
        "Color":    ["#2ECC71", "#E8452C", "#F5A623"],
    })
    fig_bar = px.bar(summary_bar, x="Category", y="Amount",
                     color="Category",
...
if not trend.empty:
    ...
    fig_trend.add_trace(go.Scatter(
        ...
        name="Revenue", line={"color":"#2ECC71","width":2}, fill="tozeroy",
        fillcolor="rgba(46,204,113,0.1)",
    ))
    fig_trend.add_trace(go.Scatter(
        x=trend["date"], y=trend["profit"],
        name="Profit", line={"color":"#F5A623","width":2},
    ))
    fig_trend.add_trace(go.Scatter(
        x=trend["date"], y=trend["total_cost"],
        name="Total Cost", line={"color":"#E8452C","width":1.5,"dash":"dot"},
    ))
...
if not pnl.empty:
    ...
    styled = pnl[show_cols].style.apply(highlight_profit, axis=1).format(
        {c: "₹{:,.0f}" for c in show_cols if c != "sku" and c != "margin_pct"}
